...
class TSP:
    ...
    def solve_B_and_B(self):
        return self.solveWithoutComments(self.get_neighbors)
        #return self.solve(self.get_neighbors)

    def solve_B_and_S(self, k=3):
        return self.solveWithoutComments(self.get_K_neighbors, k)
    ...
    def solve(self, get_neighbors_func, k=-1):
        ...
        while not pq.empty():
            ...
            if lowerBound >= self.upperBound:
                print("Pruning of the branch, not worth to go further, Cost: " + str(lowerBound) + ", Upper bound: " + str(self.upperBound))
                continue
            ...
            # If the path of the node does not include all nodes, expand the node
                ...
                for neighbor in neighbors:
                    ...
                    print("Checking if neighbor in the path already -  " + str(neighbor in path) + ", Neighbor - " + str(  neighbor) + ", Path " + str(path))
                    if neighbor in path:
                        continue
                    ...
                    if new_lowerBound < self.upperBound:
                        print("Updating queue: new Lower Bound - " + str(new_lowerBound) + ", Upper bound - " + str(self.upperBound))
    ...
    def solveWithoutComments(self, get_neighbors_func, k=-1):
        # Number of nodes in the graph
        num_nodes = self.noCities
